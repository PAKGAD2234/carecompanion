...
export default async function NewBookingPage({ searchParams }: NewBookingPageProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-8">
      ...
      <Link href="/customer/bookings" className="mb-6 inline-flex items-center gap-2 font-bold text-[#1F8F73]"><ArrowLeft size={22} strokeWidth={2} aria-hidden="true" /> กลับไปรายการคำขอ</Link>
      <Card>
        <div className="mb-8 flex items-center gap-4">
          <div className="rounded-xl bg-[#1F8F73]/10 p-3 text-[#1F8F73]"><Route size={30} strokeWidth={2} aria-hidden="true" /></div>
          <div><h1 className="text-3xl font-extrabold">สร้างคำขอบริการ</h1><p className="mt-1 text-[#2E2E2E]/75">กรอกรายละเอียดการเดินทางของคุณ</p></div>
        </div>
        ...
        {selectedCompanion ? <div className="mb-5 rounded-xl bg-[#1F8F73]/10 p-4 font-bold text-[#187A62]">เลือก Companion: {(selectedCompanion.profiles as { full_name?: string } | null)?.full_name ?? "Companion"}</div> : null}
        <form id="booking-form" action={createBooking} className="space-y-5">
          ...
          <div className="space-y-2"><label className="block text-lg font-bold" htmlFor="service_type_id">ประเภทธุระ</label><select id="service_type_id" name="service_type_id" required className="min-h-14 w-full rounded-[1.25rem] border-2 border-[#2E2E2E]/20 bg-[#FAFAF8] px-4 text-lg outline-none focus:border-[#1F8F73]"><option value="">เลือกประเภทธุระ</option>{serviceTypes?.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></div>
          ...
          <div className="space-y-2"><label className="flex items-center gap-2 text-lg font-bold" htmlFor="notes"><FileText size={21} strokeWidth={2} aria-hidden="true" /> รายละเอียดเพิ่มเติม</label><textarea id="notes" name="notes" rows={4} placeholder="เช่น ต้องช่วยพยุง มีรถเข็น หรือข้อควรระวัง" className="w-full rounded-[1.25rem] border-2 border-[#2E2E2E]/20 bg-[#FAFAF8] px-4 py-3 text-lg outline-none focus:border-[#1F8F73]" /></div>
